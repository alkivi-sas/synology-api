...
class LDAPServer(base_api_core.Core):
    """The LDAP server API.

    Currently testing @ Alkivi.

    - Get LDAP Server information
    - List objects within a Base DN on the LDAP Server
    - Create an LDAP User
    """
    def __init__(self, ip_address, port, username, password, secure=False, cert_verify=False, dsm_version=7, debug=True, otp_code=None):
        super(LDAPServer, self).__init__(ip_address, port,
    ...
    def list_users(self, offset: int = 0, limit: int = 50):
        ...
        api_path = info['path']
        req_param = {'api': api, 'action': action, 'limit': limit, "method": method, "sort_by": "name", "sort_direction": "ASC",
    ...
    def create_new_user(
        ...
        req_param = {'api': api_name, 
                'version': 1,
                'method': 'set', 
                'info': json.dumps(userinfo),
                'group': json.dumps(usergroup),
                'is_create': True,
                } 
